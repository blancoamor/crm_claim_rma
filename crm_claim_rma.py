...
class claim_line(orm.Model):
    ...
    def _warranty_limit_values(self, cr, uid, ids, invoice,
        ...
        if claim_type == 'supplier':
            suppliers = product.product_tmpl_id.seller_ids
            if not suppliers:
    ...
    def get_destination_location(self, cr, uid, product_id,
        ...
        if prod:
		seller_ids = self.pool.get('product.supplierinfo').search(cr,uid,\
			[('product_tmpl_id','=',prod.product_tmpl_id.id)],limit=1,order='sequence')
		if seller_ids:
                	return_type = self.pool.get('product.supplierinfo').browse(cr,uid,seller_ids[0]).warranty_return_partner
    ...
    def _warranty_return_address_values(self, cr, uid, ids, product, company,
        ...
        return_address = None
	seller_ids = self.pool.get('product.supplierinfo').search(cr,uid,\
		[('product_tmpl_id','=',product.product_tmpl_id.id)],limit=1,order='sequence')
        if seller_ids:
            return_address_id = self.pool.get('product.supplierinfo').browse(cr,uid,seller_ids[0]).warranty_return_address.id
            return_type = self.pool.get('product.supplierinfo').browse(cr,uid,seller_ids[0]).warranty_return_partner
        else:

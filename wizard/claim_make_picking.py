...
class claim_make_picking(orm.TransientModel):
    ...
    def action_create_picking(self, cr, uid, ids, context=None):
        ...
        if context.get('product_return'):
            ...
            common_dest_loc_id = wizard.claim_line_source_location.id
        ...
        # create picking
	stock_picking_type_id = self.pool.get('stock.picking.type').search(cr,uid,[('code','=',p_type),\
		('default_location_src_id','=',wizard.claim_line_source_location.id)])
	if not stock_picking_type_id:
		raise orm.except_orm(
			_('Error !'),
			_('Stock picking type not defined for source location'))
			
        picking_id = picking_obj.create(
            cr, uid,
            {'origin': claim.number,
             'picking_type_id': stock_picking_type_id[0],
             'move_type': 'direct',  # direct
             'state': 'draft',
        ...
        for wizard_claim_line in wizard.claim_line_ids:
            ...
            move_id = move_obj.create(
                ...
                {'name': wizard_claim_line.product_id.name_template,
                 ...
                 'product_uom': wizard_claim_line.product_id.uom_id.id or 1,
